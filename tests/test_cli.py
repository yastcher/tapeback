...
def test_summarize_command_with_provider_and_model_overrides(runner, tmp_path, monkeypatch):
...
def test_summarize_command_empty_transcript(runner, tmp_path, monkeypatch):
...
def test_status_command_with_pactl(runner, tmp_path, monkeypatch):
